...
class Payroll_earnings extends MY_Controller {
	...
	private function _next($id, $group_id=0, $url='payroll_earnings/view/') {
	...
	private function _previous($id, $group_id=0, $url='payroll_earnings/view/') {
		$payroll = new $this->Payroll_model('p');
		$payroll->setActive(1, true);
		$payroll->setCompanyId($this->session->userdata('current_company_id'),true);
			$where = new $this->Payroll_model('w');
...
*/
	...
	public function by_name($name_id,$start=0) {
		...
		$this->template_data->set('next_name', $this->_next_name($name_id, 'payroll_earnings/by_name/'));
		$this->template_data->set('previous_name', $this->_previous_name($name_id, 'payroll_earnings/by_name/'));
