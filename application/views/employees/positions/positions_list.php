...
<?php if( hasAccess('employees', 'positions', 'add') ) { ?>
  <button type="button" class="btn btn-success btn-xs pull-right ajax-modal" data-toggle="modal" data-target="#ajaxModal" data-title="Add Position" data-url="<?php echo site_url("employees_positions/add/ajax") . "?next=" . uri_string(); ?>" style="margin-right: 5px">Add Position</button>
<?php } ?>
          ...
          <table class="table table-default table-hover">
            ...
            <?php foreach($positions as $position) { ?>
              ...
              <?php if( hasAccess('employees', 'positions', 'edit') ) { ?>
                ...
                <a class="btn btn-danger btn-xs confirm_remove" href="<?php echo site_url("employees_positions/delete/{$position->id}"); ?>" data-target="#employee-group-<?php echo $position->id; ?>">Delete</button>
                </td>
              <?php } ?>
              </tr>
            <?php } ?>
