...
<a data-target="#ajaxModal" class="ajax-modal-inner <?php echo ($date->assigned) ? 'active' : ''; ?>" href="<?php echo site_url("employees_dtr/update_overtime/{$pe_id}/{$date->inclusive_date}/{$output}") . "?next=" . $this->input->get('next') . "&leave_id=" . $this->input->get('leave_id'); ?>" data-title="<?php echo date('F d, Y (l)', strtotime($date->inclusive_date)); ?>">
...
<strong><?php echo $n; ?>.</strong> <?php echo date('F d, Y (l)', strtotime($date->inclusive_date)); ?> 
<?php if ($date->assigned) { ?>
<span class="badge" style="margin-right: 5px;"><?php echo $date->assigned; ?> minutes</span>
<?php } ?>
